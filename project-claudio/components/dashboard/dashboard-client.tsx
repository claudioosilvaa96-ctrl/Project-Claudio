...
const storageKey = "project-claudio-daily-logs";
...
export function DashboardClient() {
  const [logs, setLogs] = useState<DailyLog[]>(() => normalizeLogs(getDemoDailyLogs()));
  ...
  const today = logs.at(-1) ?? createEmptyDailyLog(daysAgo(0));
  ...
  function updateToday(nextLog: DailyLog) {
    setLogs((current) => current.map((log) => (log.logDate === nextLog.logDate ? { ...nextLog, updatedAt: new Date().toISOString() } : log)));
  }
